...
- session_tracking_number - a UUID that auto-generates once per app lifecycle (first call to getSessionTrackingNumber()), persists across rounds within a session, and gets passed on every LLM request.

- duel_round_id (wire name) / duelRoundId (TS) - one per duel round (ie if you vote and then start a new round, both rounds will have a different session_id)
- x-opencode-session (in vibeduel sometimes called sessionID) - one per side in a duel round - used to tell server which side is which. 
